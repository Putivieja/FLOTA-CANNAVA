...
def sheet_to_dicts(sheet):
    """Convierte hoja de Excel a lista de dicts (primera fila = headers)."""
...
for r in comb_rows:
    ...
    pat_raw = get(r, "DOMINIO/PATENTE", "DOMINIO_PATENTE", "PATENTE", "DOMINIO")
    litros_raw = get(r, "LITROS")
    if not pat_raw or not litros_raw:
        continue
    pat = safe_str(pat_raw).upper().strip()
...
for r in estado_rows:
    pat = safe_str(get(r, "PATENTE") or "").upper().strip()
    if not pat:
        continue

    tipo = safe_str(get(r, "TIPO REPARACION", "TIPO_REPARACION", "TIPO REPARACIÓN", "TIPO") or "")
    if not tipo:
        continue
    ...
    costo_rep = safe_num(get(r, "REPUESTO 1", "REPUESTO_1", "REPUESTO1", "REPUESTO"))
    costo_mo  = safe_num(get(r, "M.O. 1", "M_O_1", "MO 1", "MO_1", "M.O.", "MO"))
    prov1     = safe_str(get(r, "PROVEEDOR 1", "PROVEEDOR_1", "PROVEEDOR1", "PROVEEDOR") or "")
    prov2     = safe_str(get(r, "PROVEEDOR 2", "PROVEEDOR_2", "PROVEEDOR2") or "")
...
for i, v in enumerate(flota_rows, 1):
    pat = safe_str(get(v, "PATENTE") or "").upper().strip()
    if not pat:
        continue
